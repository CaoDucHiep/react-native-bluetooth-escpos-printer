 ...
 */
public class RNBluetoothTscPrinterModule
  ...
  public RNBluetoothTscPrinterModule(
  ...
  public void printLabel(final ReadableMap options, final Promise promise) {
    ...
    TscCommand.SPEED speed = options.hasKey("speed")
      ? this.findSpeed(options.getInt("speed"))
      : null;
    TscCommand.ENABLE enable = options.hasKey("tear")
      ? options
          .getString("tear")
          .equalsIgnoreCase(TscCommand.ENABLE.ON.getValue())
        ? TscCommand.ENABLE.ON
        : TscCommand.ENABLE.OFF
      : TscCommand.ENABLE.OFF;
    ReadableArray texts = options.hasKey("text")
      ? options.getArray("text")
      : null;
    ReadableArray qrCodes = options.hasKey("qrcode")
      ? options.getArray("qrcode")
      : null;
    ReadableArray barCodes = options.hasKey("barcode")
      ? options.getArray("barcode")
      : null;
    ReadableArray images = options.hasKey("image")
      ? options.getArray("image")
      : null;
    ReadableArray reverses = options.hasKey("reverse")
      ? options.getArray("reverse")
      : null;
    ...
    TscCommand.DENSITY density = options.hasKey("density")
      ? this.findDensity(options.getInt("density"))
      : null;
    ReadableArray reference = options.hasKey("reference")
      ? options.getArray("reference")
      : null;
    ...
    for (int i = 0; texts != null && i < texts.size(); i++) {
      ...
      int x = text.getInt("x");
      int y = text.getInt("y");
      TscCommand.FONTTYPE fonttype =
        this.findFontType(text.getString("fonttype"));
    ...
    if (barCodes != null) {
      for (int i = 0; i < barCodes.size(); i++) {
        ...
        TscCommand.ROTATION rotation =
          this.findRotation(bar.getInt("rotation"));
        String code = bar.getString("code");
        TscCommand.BARCODETYPE type =
          this.findBarcodeType(bar.getString("type"));
        TscCommand.READABLE readable =
          this.findReadable(bar.getInt("readable"));
        tsc.add1DBarcode(
          x,
          y,
          type,
  ...
  @Override
  public void onBluetoothServiceStateChanged(
    int state,
    Map<String, Object> boundle
  ) {}
}
